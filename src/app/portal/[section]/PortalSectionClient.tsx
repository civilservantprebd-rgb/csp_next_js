...
export default function PortalSectionClient({
  ...
  const [config, setConfig] = useState<AppConfigData | null>(
    initialData
      ? {
          courses: [],
          subjects: [],
  ...
  const [initialSubmissions, setInitialSubmissions] = useState<Submission[] | null>(initialData?.submissions || null);
  const [bootLoading, setBootLoading] = useState(!initialData);
  ...
  useEffect(() => {
    ...
    // Skip fetching if initialData is present and matches the current user
    // (Meaning the Server Component successfully resolved it via cookies)
    if (initialData && (initialData.studentId === gUser.uid)) {
