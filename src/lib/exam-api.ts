...
/**
 * অ্যাপে পরীক্ষা দেখানোর সময়সীমা — **শুরুর ১২ ঘণ্টা আগে থেকে**।
 *
 * ব্যবহারকারীর নির্দেশ (২০২৬-০৯-২৩): *"যে এক্সামগুলো ১২ ঘন্টার মধ্যে শুরু হবে না
 * বা লাইভ না, সেগুলো ব্যাকএন্ড থেকে অ্যাপে দেখা যাবে না ... লিডারবোর্ড, কোর্স,
 * কোথাও না। যে এক্সাম হয়ে গিয়েছে বা লাইভ আছে বা ১২ ঘন্টার মধ্যে হবে কেবল
 * সেগুলোই দেখা যাবে।"*
 */
export const APP_VISIBILITY_HORIZON_MS = 12 * 60 * 60 * 1000;

/**
 * এই পরীক্ষাটি অ্যাপে দেখানো হবে কি না — **সব তালিকায় একটাই নিয়ম**।
 *
 * ── নিয়ম ──
 * দেখা যাবে যদি:
 *   ১. **লাইভ** — এখনই চলছে, অথবা
 *   ২. **হয়ে গেছে** — শুরু হয়ে গেছে (ফলাফল · মেধা তালিকা দেখার জন্য), অথবা
 *   ৩. **১২ ঘণ্টার মধ্যে শুরু হবে**।
 * লুকানো হবে: যার শুরু এখনো **১২ ঘণ্টার বেশি দূরে**।
 *
 * ── কেন এত সহজে লেখা যায় ──
 * উপরের তিনটা দশা আসলে **শুরুর সময়ের একটাই প্রশ্ন**: `startMs - now <= 12h`।
 * (শুরু হয়ে গেলে পার্থক্যটা ঋণাত্মক — লাইভ ও শেষ হওয়া দুটোই এর ভেতরে পড়ে।)
 * তাই আলাদা করে `isLive`/`isClosed` মেলাতে হয় না — দুই শাখায় ভাগ করলে একদিন
 * কোনো এক শাখায় ভুল হলে ঠিক ওই পর্দাতেই পরীক্ষা হারিয়ে যেত।
 *
 * ⚠️ **সময়সূচি নেই = সর্বদা-খোলা প্র্যাকটিস** — ওটা এখনই দেওয়া যায়, তাই
 * লুকানোর কিছু নেই (গেট করার মতো "শুরু" বলেই কিছু নেই)।
 *
 * ⚠️ এটা **প্রবেশ-নিয়ন্ত্রণ নয়**, কেবল তালিকা-ছাঁকনি। আসল প্রতিরোধ
 * `assertExamWindow` (শুরুর আগে প্রশ্ন নয়) ও `assertExamAccess`-এ — ওগুলো
 * অপরিবর্তিত।
 */
export function isExamVisibleInApp(exam: Exam, nowMs: number): boolean {
  const { startMs } = examWindows(exam, nowMs);
  if (startMs === null) return true;
  return startMs - nowMs <= APP_VISIBILITY_HORIZON_MS;
}
